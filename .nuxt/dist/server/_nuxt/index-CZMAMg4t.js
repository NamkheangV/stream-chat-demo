    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(chats), (msg) => {
        ...
        ssrRenderList(msg.badges, (badge, i) => {
          _push(`<span class="badge-slot" data-v-8583c229>${badge ?? ""}</span>`);
        });
        _push(`<!--]--><span class="username" style="${ssrRenderStyle({ color: msg.nameColor })}" data-v-8583c229>${ssrInterpolate(msg.user)}</span></div><div class="chat-bubble" data-v-8583c229>${msg.html ?? ""}</div></div>`);
      });
      _push(`</div></div>`);
    };
  }
...
//# sourceMappingURL=index-CZMAMg4t.js.map
